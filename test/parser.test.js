...
test("Check for defines (simple)", () => {
    ...
    let defs = parser.parseCommandDefine(lines);
...
test("Check for defines (testCase)", () => {
    ...
    let defs = parser.parseCommandDefine(lines);
    expect(Object.keys(defs).length).toEqual(2);
...
test("Check for defines (testCase)", () => {
    let lines = parser.lines(testCase);
    parser.defines = parser.parseCommandDefine(lines);
